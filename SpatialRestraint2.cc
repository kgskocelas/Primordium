...
struct World {
  ...
  void ResetOrganism(Organism & org) {
    org.resources = 1;
    org.threshold = threshold_dist.PickRandom(random);
  }
  ...
  size_t TestMulticell() {
    ...
    while (num_orgs < mc_size) {
      ...
      for (size_t pos = 0; pos < mc_size; pos++) {
        ...
        if (orgs[pos].resources) {
          // If so, give resource and see if it replicates.
          if (++orgs[pos].resources == orgs[pos].threshold) {
            ResetOrganism(orgs[pos]);
