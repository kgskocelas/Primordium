...
for(size in size_vec){
  size_mask = df$mc_size == size
  cat('Size: ', size, '\n')
  for(batch_id in unique(df$batch_id)){
    mask = size_mask & df$batch_id == batch_id
    cat('Batch: ', batch_id, '\n\t')
    for(gen in unique(df$generation)){
      df_summary[nrow(df_summary) + 1,] = c(size, gen, weighted.mean(0:100, df[mask & df$generation == gen,]$frac_of_pop), batch_id)
      cat(gen, ' ')
    }
    cat('\n')
  }
...
ggplot(df_summary[df_summary$generation == 10000,], aes(x = size_factor, y = restraint_value ))  +
  ...
  theme(strip.background = element_rect(fill = '#dddddd')) +
  theme(legend.position = 'none') 
ggsave('./simulation/plots/final/evolved_boxplot.png', units = 'in', width = 6, height = 6) 
ggsave('./simulation/plots/final/evolved_boxplot.pdf', units = 'in', width = 6, height = 6)
