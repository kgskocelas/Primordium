...
df_fitness = data.frame(data = matrix(nrow = 0, ncol = 4))
colnames(df_fitness) = c('mc_size', 'num_ones', 'fitness', 'batch_id')
for(mc_size in size_vec){
  size_mask = df_summary$cells_side == mc_size
  for(batch_id in unique(df_summary$batch_id)){
    batch_mask = size_mask & df_summary$batch_id == batch_id
    for(ones in unique(df_summary$ancestor_1s)){
      df_fitness[nrow(df_fitness) + 1,] = c(mc_size, ones, df_summary[batch_mask & df_summary$ancestor_1s == ones,]$mean_rep_rate, batch_id)
    }
  }
}
...
for(size_idx in 1:length(size_vec)){
  ...
  for(batch_id in unique(df_fitness$batch_id)){
    ...
    fit_size_mask = df_fitness$mc_size == size & df_fitness$batch_id == batch_id
    ...
    # Write out data
    write.csv(df_density, paste0('simulation/data/size_', size, '__batch_', batch_id, '.csv'))
    ...
    ggsave(paste0('./simulation/plots/sim/density_over_time__size_', size, '__batch_', batch_id, '.png'), units = 'in', width = 6, height = 6)
