...
for(mc_size in size_vec){
  for(batch_id in 1:num_batches){
    size_mask = df_summary$cells_side == mc_size & df_summary$batch_id == batch_id
    for(ones in unique(df_summary$ancestor_1s)){
      df_fitness[nrow(df_fitness) + 1,] = c(mc_size, ones, df_summary[size_mask & df_summary$ancestor_1s == ones,]$mean_rep_rate, batch_id)
    }
  }
}
...
for(size_idx in 1:length(size_vec)){
  ...
  for(batch_id in 1:num_batches){
    batch_mask = fit_size_mask & df_fitness$batch_id == batch_id
    ...
    fit_vec = df_fitness[batch_mask,]$fitness 
