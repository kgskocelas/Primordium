...
# Generate the jobs!
python3 ${SR_ROOT_DIR}/experiments/scripts/evolution_job_prep.py --executable_path ${ROOT_DIR}/application/bin/SpatialRestraint --output_dir ${SR_EVO_OUTPUT_DIR} --job_dir ${SR_EVO_JOB_DIR} --distribution_dir ${SR_TIMING_DIR}/data --ones ${SR_EVO_ONES} --gens ${SR_EVO_GENS} --cost ${SR_COST} --mc_size ${SR_MC_SIZE} --pop_size ${SR_EVO_POP_SIZE} --mut_rate ${SR_EVO_MUT_RATE} --samples ${SR_SAMPLES} --reps ${SR_EVO_REPS} --num_jobs ${SR_EVO_JOBS} --seed_offset ${SR_EVO_SEED_OFFSET} --time ${SR_EVO_TIME} --memory ${SR_EVO_MEMORY} ${SR_ONE_CHECK_LOCAL} ${SR_INFINITE_LOCAL}
