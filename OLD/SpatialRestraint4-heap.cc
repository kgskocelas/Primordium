...
struct World {
  ...
  emp::vector<Cell> cell_heap;  ///< Cells waiting to replicate.
  ...
  void Debug() {
    std::cout << "DEBUG: ... cell_heap=[";
    for (auto x : cell_heap) { std::cout << " " << x.id << "(" << x.repro_time << ")"; }
    std::cout << "]\n";
    std::cout << "           cells: ";
  ...
  Results TestMulticell() {
    ...
    while (num_cells < cells.size()) {
      emp_assert(cell_heap.size() > 0);

      const Cell & parent = emp::HeapExtract(cell_heap);
